...
The app is a standard Next.js deployment (built with `--webpack`) plus a Postgres. `vercel.json` ships a cron every 30 minutes hitting `/api/cron/drain` (set `CRON_SECRET`); webhooks drain the queue inline, so the cron only carries scheduled rescans, the exposure counts and mail. Set `REGISTRY_SOURCE` to a raw URL serving the registry JSON, e.g. `https://raw.githubusercontent.com/aidep-dev/aidep-registry/master/registry`. The registry repo needs no deployment: its daily GitHub Actions cron (`.github/workflows/poll.yml`) opens a review PR against itself when a provider page changes; merging the PR is the approval step.

Two things worth knowing before you pick hosts. Vercel's Hobby plan is non-commercial only, so charging anyone means Pro. And Vercel Postgres no longer exists (those databases moved to Neon in Dec 2024). aidep.dev runs on Neon: the free plan gives 100 compute hours a month per project and suspends the database when they run out. Every cron tick wakes the compute for at least the five-minute idle window, so a 10-minute cron burned about 110 compute hours a month and the cron runs every 30 minutes instead; plan on the paid tier once real repos install.
