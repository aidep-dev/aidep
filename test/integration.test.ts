...
describe("cron drain route", () => {
  ...
  it("enqueues rescans for stale repos only, then drains them", async () => {
    ...
    expect(await res.json()).toEqual({ enqueued: 1, registryTriggered: 0, exposure: { updated: 0, skipped: 0 }, ran: 1, failed: 0 });
    ...
    expect(await res2.json()).toEqual({ enqueued: 0, registryTriggered: 0, exposure: { updated: 0, skipped: 0 }, ran: 0, failed: 0 });
  });
  ...
  it("does not double-enqueue when a scan job is already queued", async () => {
    ...
    expect(await res.json()).toEqual({ enqueued: 0, registryTriggered: 0, exposure: { updated: 0, skipped: 0 }, ran: 1, failed: 0 });
  });
