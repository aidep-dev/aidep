...
export function buildOnboardingPr(input: OnboardInput): OnboardingPrContent {
  ...
  const pre = [
    ...
    "- `evals`: false by default. When true, migration PRs also ship an evals/ pack + a GitHub Actions workflow file that runs your prompts on old vs new model in YOUR CI with YOUR keys; you review the extracted test cases in the PR before anything runs. Drafting those cases sends the affected files to Anthropic once each, using our key; credential-named files (.env*, key, pem, p12, pfx, secrets, credentials) are never sent and key-shaped strings in the rest are redacted first. Leave `evals` off and your code never leaves the scan.",
    "",
